...
namespace ViajeFacilApi.Controllers
{
    ...
    public class RotaController : ControllerBase
    {
        ...
        {
            this.servico = new RotaService(context);
        }
        ...
        public ActionResult<List<RotaPoco>> GetAll(int? take = null, int? skip = null)
        {
            try
            {
                List<RotaPoco> lista = this.servico.Listar(take, skip);
                return Ok(lista);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<RotaPoco> Post([FromBody] RotaPoco poco)
        {
            try
            {
                RotaPoco novoPoco = this.servico.Inserir(poco);
                return Ok(novoPoco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<RotaPoco> Put([FromBody] RotaPoco poco)
        {
            try
            {
                RotaPoco novoPoco = this.servico.Alterar(poco);
                return Ok(novoPoco);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<RotaPoco> DeleteById(long chave)
        {
            try
            {
                RotaPoco poco = this.servico.Excluir(chave);
                return Ok(poco);
            }
            catch (Exception ex)
            {
